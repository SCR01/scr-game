...
//  Added the new showOn value in the navItems to hide or show the each items based on ds
const navItems = [
  { name: "Nexus", href: "#about", showOn: "all" },
  { name: "Vault", href: "#features", showOn: "all" },
  { name: "Prologue", href: "#story", showOn: "desktop" },
  { name: "About", href: "#about", showOn: "desktop" },
  { name: "Contact", href: "#contact", showOn: "desktop" },
  { name: "Products", href: "#products", showOn: "wide" },
];
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)} 
        className="px-2 sm:px-4 py-2 bg-gradient-to-r from-yellow-400 to-yellow-600 text-black font-bold rounded-lg hover:from-yellow-300 hover:to-yellow-500 transition-all duration-300 shadow-lg hover:shadow-yellow-400/25 disabled:opacity-50"
        disabled={isLoading}
      >
...
const Navbar = ({ gameTitles = [] }) => {
  ...
  // Updated All the req tailwind and fixed bugs
  return (
    <div
      ref={navContainerRef}
      className="fixed inset-x-0 top-0 z-50  h-19  sm:pt-0 pt-2 sm:pb-0 pb-2 sm:h-20 transition-all duration-700  bg-black/50 backdrop-blur-sm"
    >
      <header className="w-full  gap-2">
        <nav className="flex items-center justify-between  px-2 py-2 sm:p-4">
          {/* Left Section */}
          <div className="flex items-center  gap-0  sm:gap-7">
            ...
            <Button
              ...
              containerClass="bg-blue-50 2xl:flex hidden items-center justify-center gap-1"
            />
        ...
        item.showOn === "desktop"
          ? "hidden xl:inline" // desktop only
          : item.showOn === "mobile"
          ? "md:hidden"        // mobile only
          : item.showOn === "wide"
          ? "min-[1535px]:hidden" // hide ≥1500px
          : ""                 // all screens
      }`}
            ...
            {/* Mobile Menu Toggle */}
            <button
              className="xl:hidden ml-2 p-2 text-white"
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="xl:hidden bg-gray-900/90 backdrop-blur-sm p-4 space-y-3">
            {navItems.map((item, i) => (
              <a
                key={i}
                href={item.href}
